...
//
//  Created by Himanshi Godara on 30/08/25.
//
...
class ViewController: UIViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate, MFMailComposeViewControllerDelegate {
    ...
    @IBAction func safariButton(_ sender: Any) {
        if let url=URL(string: "https://wwwapple.com"){
    ...
    @IBAction func emailButton(_ sender: Any) {
        ...
        if let image = imageView.image, let jpegData =
            image.jpegData (compressionQuality: 0.9) { mailComposer.addAttachmentData (jpegData, mimeType:
